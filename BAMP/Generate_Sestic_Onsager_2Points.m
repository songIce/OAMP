function Generate_Sestic_Onsager_2Points(alphagrid,epsl,niter,PCA_init,alpha1)
...
muSE_all = zeros(niter, length(alphagrid));
sigma2SE_all = zeros(niter, length(alphagrid));
Onsager_correct_all = zeros(niter, niter,length(alphagrid));
...
%                     DeltaMAT(i, 5*t+1) = DeltaMAT(5*t+1, i);
        ...
        if abs(MSE(t+1,j) - 0.5) < 1e-4 || abs(MSE(t+1,j) - MSE(t,j)) < 1e-7
            for kk = t+2 : niter
                scal_all(kk, j) = scal_all(t+1, j); 
                MSE(kk, j) = MSE(t+1, j);
            end
    ...
    % save
    muSE_all(:,j) = muSE;
    sigma2SE_all(:,j) = sigma2SE;
    Onsager_correct_all(:,:,j) = Onsager_correct;
    
end

muSE = muSE_all;
sigma2SE = sigma2SE_all;
Onsager_correct = Onsager_correct_all;
BAMP_SE_MSE = MSE;  
BAMP_SE_overlap = scal_all;
save SE_sestic_Onsager.mat Onsager_correct BAMP_SE_MSE BAMP_SE_overlap muSE sigma2SE;

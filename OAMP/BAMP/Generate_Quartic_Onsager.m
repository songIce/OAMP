function Generate_Quartic_Onsager(u,alphagrid,epsl,niter,rho,PCA_init)
        ...
        for t = 1 : niter-1 %beginning of the iterations
            ...
            %covariance between two iterates at steps that are multiples of 3
                ...
                %fun1 = @(x, y) 1/(2*pi*sqrt(det(Sigma1))) * exp(-1/2 * (( invS(1, 1) * x.^2 + invS(2, 2) * y.^2 + 2*invS(1, 2)*x.*y))) ...
                    %.* tanh(muSE(t,j)^2/sigma2SE(t,j) + muSE(t,j)/sigma2SE(t,j) * x) ...
                    %.* tanh(muSE(t1,j)^2/sigma2SE(t1,j) + muSE(t1,j)/sigma2SE(t1,j) * y);
                ...
                fun0 = @(x, y) rho*1/(2*pi*sqrt(det(Sigma1))) * exp(-1/2 * (( invS(1, 1) * x.^2 + invS(2, 2) * y.^2 + 2*invS(1, 2)*x.*y))) ...
                    .* tanh(muSE(t,j)/sigma2SE(t,j)/sqrt(rho) * x) ...
                    ./ (rho+(1-rho).* sech(muSE(t,j)/sigma2SE(t,j)/sqrt(rho) * x) .*exp(muSE(t,j)^2/sigma2SE(t,j)/2/rho))...
                    .* tanh( muSE(t1,j)/sigma2SE(t1,j)/sqrt(rho) * y)...
                    ./ (rho+(1-rho).* sech( muSE(t1,j)/sigma2SE(t1,j)/sqrt(rho) *y).*exp(muSE(t1,j)^2/sigma2SE(t1,j)/2/rho));
    
                fun1 = @(x, y) rho*1/(2*pi*sqrt(det(Sigma1))) * exp(-1/2 * (( invS(1, 1) * x.^2 + invS(2, 2) * y.^2 + 2*invS(1, 2)*x.*y))) ...
                   .* tanh(-muSE(t,j)^2/sigma2SE(t,j)/rho + muSE(t,j)/sigma2SE(t,j)/sqrt(rho) * x) ...
                    ./ (rho+(1-rho).* sech(-muSE(t,j)^2/sigma2SE(t,j)/rho + muSE(t,j)/sigma2SE(t,j)/sqrt(rho) * x) .*exp(muSE(t,j)^2/sigma2SE(t,j)/2/rho))...
                    .* tanh(-muSE(t1,j)^2/sigma2SE(t1,j)/rho + muSE(t1,j)/sigma2SE(t1,j)/sqrt(rho) * y)...
                    ./ (rho+(1-rho).* sech(-muSE(t1,j)^2/sigma2SE(t1,j)/rho + muSE(t1,j)/sigma2SE(t1,j)/sqrt(rho) *y).*exp(muSE(t1,j)^2/sigma2SE(t1,j)/2/rho));
            ...
            phi_int = muSE(t,j)/sigma2SE(t,j) * (rho*int3+(1-rho)*int4 - rho*(int1 + int2)/2-(1-rho)*int0);
            ...
            for ell = 1 : 2
                ...
                for t1 = 1 : t
                    ...
                    fun0 = @(x, y) sqrt(rho)*1/(2*pi*sqrt(det(Sigma1))) * exp(-1/2 * (( invS(1, 1) * x.^2 + invS(2, 2) * y.^2 + 2*invS(1, 2)*x.*y))) ...
                        .* tanh( muSE(t1,j)/sigma2SE(t1,j)/sqrt(rho) * x) ...
                        ./ (rho+(1-rho).* sech(muSE(t1,j)/sigma2SE(t1,j)/sqrt(rho) * x) .*exp(muSE(t1,j)^2/sigma2SE(t1,j)/2/rho))...
                        .* y;
    
                    fun1 = @(x, y) sqrt(rho)*1/(2*pi*sqrt(det(Sigma1))) * exp(-1/2 * (( invS(1, 1) * x.^2 + invS(2, 2) * y.^2 + 2*invS(1, 2)*x.*y))) ...
                        .* tanh(-muSE(t1,j)^2/sigma2SE(t1,j)/rho + muSE(t1,j)/sigma2SE(t1,j)/sqrt(rho) * x) ...
                        ./ (rho+(1-rho).* sech(-muSE(t1,j)^2/sigma2SE(t1,j)/rho + muSE(t1,j)/sigma2SE(t1,j)/sqrt(rho) * x) .*exp(muSE(t1,j)^2/sigma2SE(t1,j)/2/rho))...
                        .* y;
                    ...
                    %fun2 = @(x, y) 1/(2*pi*sqrt(det(Sigma1))) * exp(-1/2 * (( invS(1, 1) * x.^2 + invS(2, 2) * y.^2 + 2*invS(1, 2)*x.*y))) ...
                        %.* tanh(-muSE(t1,j)^2/sigma2SE(t1,j) + muSE(t1,j)/sigma2SE(t1,j) * x) ...
                        %.* y;
    ...
    save SE_quartic_Onsager.mat Onsager_correct BAMP_SE_MSE BAMP_SE_overlap muSE sigma2SE;
